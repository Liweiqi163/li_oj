...
//      异步执行代码沙箱去判题
    ...
    /**
     * 获取查询包装类(用户可能根据哪些字段来查询，根据前端传来的请求对象，得到mybatis框架支持的QueryWrapper类)
     *
...
//      如果查询的用户不当前用户或者不是管理员，则不可以看到提交的代码
        Long id = loginUser.getId();
        if(questionSubmit.getUserId()!=id && userService.isAdmin(loginUser)){
    ...
    public Page<QuestionSubmitVO> getQuestionSubmitVOPage(Page<QuestionSubmit> questionSubmitPage, User loginUser) {
        ...
        Page<QuestionSubmitVO> questionVOPageVO = new Page<>(questionSubmitPage.getCurrent(), questionSubmitPage.getSize(), questionSubmitPage.getTotal());
        ...
        questionVOPageVO.setRecords(collect);
        return questionVOPageVO;
    }
}
